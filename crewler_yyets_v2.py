...
favorlistfilename = 'yy2list\yyfavor_list_'+time.strftime('%Y%m%d')+'.txt' #收藏裡的影片資料連結清單，給程式做參照用
favortitlefilename = 'yy2list\yyfavor_title_list_'+time.strftime('%Y%m%d')+'.txt' #影片裡的片名清單，給使用者作參考用
userseedlistfilename = 'yy2list\yuser_seedlist_'+time.strftime('%Y%m%d')+'.txt' #種子清單，提供給使用者作參考用
usered2klistfilename = 'yy2list\yuser_ed2klist_'+time.strftime('%Y%m%d')+'.txt' #eDonkey種子清單，提供給使用者作參考用
usermagnetlistfilename = 'yy2list\yuser_magnetlist_'+time.strftime('%Y%m%d')+'.txt' #磁力連結清單，提供給使用者作參考用
seedlistfilename = 'yy2list\seedlist_'+time.strftime('%Y%m%d')+'.txt' #程式執行當日爬取到的種子清單，給程式做比對用
newseedlistfilename = 'yy2list\yynewseedlist_'+time.strftime('%Y%m%d')+'.txt' #比對完成之後，還未使用過的種子清單，可提供給下載程式做使用
                ...
                for block2 in soup2.select('a.f3'):
                    ...
                    if '/resource/list/' in source_link: #這link被隱藏，利用爬蟲將其爬取出來
                        with open(favorlistfilename, 'a') as favor:
                            ...
                            favor.close()
            print(block.text)
            with open(favortitlefilename, 'a', errors='ignore') as title:
...
def make_seedlist(favorlistfilename = favorlistfilename, userseedlistfilename = userseedlistfilename, seedlistfilename = seedlistfilename, usered2klistfilename = usered2klistfilename, usermagnetlistfilename = usermagnetlistfilename, cookies = make_cookie()):
    ...
    for list in favorlist:
        ...
        print(list)
        henk = '----------------------' + '\n' + list + '\n' + soup.title.text + '\n'
        with open(userseedlistfilename, 'a', errors='ignore') as seedlist: #開啟檔案，假如檔案不存在則新增檔案
            seedlist.write(henk)
            seedlist.close()
        with open(usered2klistfilename, 'a', errors='ignore') as seedlist: #開啟檔案，假如檔案不存在則新增檔案
            seedlist.write('----------------------' + '\n' + list + '\n' + soup.title.text + '\n')
            seedlist.close()
        with open(usermagnetlistfilename, 'a', errors='ignore') as seedlist: #開啟檔案，假如檔案不存在則新增檔案
            seedlist.write('----------------------' + '\n' + list + '\n' + soup.title.text + '\n')
            seedlist.close()
        ...
        """
        print('+++++++MP4+++++++')
        with open(userseedlistfilename, 'a', errors='ignore') as seedlist:
            seedlist.write('+++++++MP4+++++++' + '\n')
            seedlist.close()
        with open(usered2klistfilename, 'a', errors='ignore') as seedlist:
            seedlist.write('+++++++MP4+++++++' + '\n')
            seedlist.close()
        with open(usermagnetlistfilename, 'a', errors='ignore') as seedlist:
            seedlist.write('+++++++MP4+++++++' + '\n')
            seedlist.close()
            ...
            for block2 in block1.select('div.fr'):
                for block3 in block2.select('[type=ed2k]'):
                    ...
                    with open(userseedlistfilename, 'a', errors='ignore') as seedlist:
                        seedlist.write(url+'\n')
                        seedlist.close()
                    with open(seedlistfilename, 'a', errors='ignore') as seedlist:
                        seedlist.write(url+'\n')
                        seedlist.close()
                    with open(usered2klistfilename, 'a', errors='ignore') as seedlist:
                        seedlist.write(url+'\n')
...
def defineseedlist(seedlistfilename = seedlistfilename, seedlibfilename = seedlibfilename, newseedlistfilename = newseedlistfilename):
    ...
    :return: none
    """
    seed_a = [line.strip() for line in open(seedlistfilename, errors='ignore')]
    open(seedlistfilename).close()
    seed_lib = [line.strip() for line in open(seedlibfilename, errors='ignore')]
    ...
    for list_new in seed_new:
        with open(newseedlistfilename, 'a', errors='ignore') as seedfilenew: #將未放入種子庫的種子寫入當日新種子檔
            seedfilenew.write(list_new + '\n')
            seedfilenew.close()

    for list_new in seed_new:
        with open(seedlibfilename, 'a', errors='ignore') as seedfilelib: #將未放入種子庫的種子寫入種子庫
